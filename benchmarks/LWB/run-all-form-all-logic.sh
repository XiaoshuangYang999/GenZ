...
# bench families for each group6
group_formulas() {
  case "$1" in
    k_p)  echo "k_branch_p k_d4_p k_dum_p k_grz_p k_lin_p k_path_p k_ph_p k_poly_p k_t4p_p" ;;
    k_n)  echo "k_branch_n k_d4_n k_dum_n k_grz_n k_lin_n  k_pkl÷ath_n k_ph_n k_poly_n k_t4p_n" ;;
...
SIZE_RAW="all_size${SIZE_LIMIT}_${TIMEOUT}s_raw.csv"
SIZE_PERCENT="all_size${SIZE_LIMIT}_${TIMEOUT}s_percent.csv"

echo "group,logic,prover,formula,index,size,runtime,res" > "$SIZE_RAW"

# Loop：group -> ljbogic -> prover -> formula -> index
