...
main = do
  ...
  size <- getLine
  content <- readFile $ "benchmarks/LWB/lwb_s4/" ++ name1 ++ ".txt." ++ size ++ ".intohylo"
  case parseFormM (alexScanTokens content) of
    Left e -> print e
    Right f -> do
      putStr "GenZ: "
      r <- timeout limit (evaluate $ isProvableZ sfour (neg f))
      case r of
        Nothing -> putStrLn "Timeout"
        Just b  -> print b
      putStr "GenT: "
      r2 <- timeout limit (evaluate $ isProvableT sfour (neg f))
      case r2 of
        Nothing -> putStrLn "Timeout"
        Just b  -> print b
